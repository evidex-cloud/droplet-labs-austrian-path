...
# Taxes, Subsidies & Regulation: The Hidden Costs
...
Keep the market from [[price-controls|Stage 8.2]]: demand \(Q_{d} = 100 - 2P\), supply \(Q_{s} = -20 + 2P\), equilibrium \(P^{*} = 30\), \(Q^{*} = 40\). Now levy a tax of 10 per unit, to be remitted by sellers.

The price buyers pay, \(P_{b}\), and the price sellers keep, \(P_{s}\), are separated by 10: \(P_{b} = P_{s} + 10\). The market clears when what buyers want at \(P_{b}\) equals what sellers want to sell at \(P_{s}\):

$$
\begin{aligned}
100 - 2(P_{s} + 10) &= -20 + 2P_{s} \\
80 - 2P_{s} &= -20 + 2P_{s} \;\rightarrow\; P_{s} = 25,\ P_{b} = 35,\ Q = 30 \\[6pt]
\text{Tax revenue} &= 10 \times 30 = 300 \\
\text{Deadweight loss} &= \tfrac{1}{2} \times 10 \times (40 - 30) = 50
\end{aligned}
$$

Three conclusions:

- **Incidence has nothing to do with the law.** Buyers go from paying 30 to paying 35 (5 more); sellers go from keeping 30 to keeping 25 (5 less). Rewrite the law so buyers remit the tax and the resulting \(P_{b}\), \(P_{s}\) and Q are identical. Who “remits” is bookkeeping; who **bears** the tax is set by the two sides' elasticities. Here the slopes are equal, so the split is even; if demand were more inelastic (must-have goods), buyers would bear more.
...
The reason goes back to the typology of [[intervention-logic|Stage 8.1]]: a tax is a **binary intervention** — a coerced exchange between the government and the taxpayer. And it has two motions, not one:
...
Rothbard borrows Franz Oppenheimer's distinction (*The State*, 1908): wealth is acquired by two means — the **economic means** (production and voluntary exchange) and the **political means** (coerced transfer). Taxation is the institutionalised form of the second. This is not to say government spending produces nothing — roads and courts do get built — but that **the mechanism for judging whether those outputs are worth their cost (profit and loss, [[profit-loss|Stage 6.3]]) does not exist here.** The taxpayer cannot decline to buy, so the government has no way to learn whether what it provides is worth what it charges. [[economics-of-state|Stage 8.4]] returns to this with Mises's *Bureaucracy*.
...
\begin{aligned}
P_{b} &= P_{s} - 10 \\
\rightarrow\; 100 - 2P_{b} &= -20 + 2(P_{b} + 10) \\
\rightarrow\; P_{b} &= 25,\ P_{s} = 35,\ Q = 50 \\[6pt]
\text{Fiscal cost} &= 10 \times 50 = 500 \\
\text{Deadweight loss} &= \tfrac{1}{2} \times 10 \times (50 - 40) = 50
\end{aligned}
$$
...
- **Brands and reputation**: a brand built over twenty years is capital that a single incident can destroy; that is why large firms run quality control where no rule requires it. [[algorithms-order|Stage 16.2]] shows how platform rating systems extend this mechanism to millions of small sellers.
...
**Step three: apply a procedure.** This previews the five-step method of [[five-step-analysis|Stage 14.1]]. Faced with any regulation, ask in order:
...
- **Which margins will adjust?** Price, quality, hours, entry, automation, relocation (the homework of [[price-controls|Stage 8.2]]).
...
## @quiz
1. Demand \(Q_{d} = 100 - 2P\), supply \(Q_{s} = -20 + 2P\); a tax of 10 per unit is levied on sellers. What price do buyers pay, and what price do sellers keep?
   - [ ] 40 and 30
   - [x] 35 and 25
   - [ ] 30 and 20
   - [ ] 35 and 35
   > \(P_{b} = P_{s} + 10\); substituting into the clearing condition gives \(P_{s} = 25\), \(P_{b} = 35\), \(Q = 30\). Buyers pay 5 more, sellers keep 5 less — equal elasticities, so an even split; make buyers the remitter and the numbers are identical.

2. Same market: the tax rises from 10 to 20. Deadweight loss goes from 50 to what?
   - [ ] 100
   - [ ] 150
   - [x] 200
   - [ ] 400
   > At a tax of 20, Q falls to 20, and DWL = ½ × 20 × (40 − 20) = **200**. Double the tax, quadruple the loss — both sides of the triangle stretch.

3. What does Yandle's “Bootleggers and Baptists” describe?
   - [ ] Regulation is always driven by moral motives
   - [x] A regulation typically needs both a group supplying the moral case and a group that profits from it; they need not collude, but neither suffices alone
   - [ ] Bootleggers always oppose regulation
   - [ ] Religious groups are behind all regulation
   > Prohibition's supporters: Baptists (sincerely believing drink harmful) and bootleggers (whose business the ban created). When analysing any regulation, first ask who is making money.

4. A new rule costs every firm a fixed 1 million a year. For an incumbent with 500 million in revenue and a startup with 5 million, what share of revenue does it consume?
   - [ ] 2% for both
   - [x] 0.2% and 20%
   - [ ] 2% and 0.2%
   - [ ] 0.2% and 2%
   > 1 million / 500 million = **0.2%**; 1 million / 5 million = **20%**. A fixed compliance cost is life-or-death for the small firm and a rounding error for the giant — which is why regulation is so often a moat the incumbent asked for.

5. Which of the following is **not** part of the Austrian reply to “drugs need ex ante regulation because the harm is irreversible”?
   ...
   - [x] Drug safety does not matter; the market automatically weeds out bad drugs
   > Austrians never deny that the risk is real; their reply points to market alternatives, admits the detail of private failures, and draws the regulation's unseen cost (delay). “It does not matter” is a straw man.

## @further
- [Rothbard, Power and Market, Ch. 4 “Binary Intervention: Taxation” — the case that no tax is neutral](https://mises.org/library/book/man-economy-and-state-power-and-market)
- [George Stigler, “The Theory of Economic Regulation” (Bell Journal of Economics, 1971) — the original capture-theory paper](https://www.jstor.org/stable/3003160)
- [Bruce Yandle, “Bootleggers and Baptists: The Education of a Regulatory Economist” (Regulation, 1983)](https://www.cato.org/sites/cato.org/files/serials/files/regulation/1983/5/v7n3-3.pdf)
- [Mises, Human Action, Ch. 28 “Interference by Taxation” and Ch. 29 “Restriction of Production”](https://mises.org/library/book/human-action)
- [Econlib Encyclopedia: Occupational Licensing (S. David Young)](https://www.econlib.org/library/Enc/OccupationalLicensing.html)
